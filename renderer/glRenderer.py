...
# glRenderer makes window named GL Renderer, which shows rendered image of :
# original img + rendered smpl in front of the img
# window with bbox + rendered img actually gets the rendered img from the window GL Renderer, so it is not possible to
# eliminate gl renderer window only, if you are just using this code.
class glRenderer:
    ...
    def __init__(self, width=640, height=480, name='GL Renderer',
        ...
        if _glut_window is None:
            ...
            # keyboardFunc enables glut window to take the keyboard input.
            # this, in fact, is the function that makes esc-exit / space-shape / o-obj input possible
            # in GL_Renderer window.
